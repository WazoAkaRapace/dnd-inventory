...
async function main() {
  ...
  console.log(
    `[translate-spells] ${spells.length} spells, fetching ${AIDEDD_SLUGS.length} French pages from AideDD.org…`,
  );
  ...
  for (const vfSlug of AIDEDD_SLUGS) {
    ...
    if (fetched % 50 === 0) {
      console.log(
        `[translate-spells] progress: ${fetched}/${AIDEDD_SLUGS.length} fetched, ${matched} matched`,
      );
    }
