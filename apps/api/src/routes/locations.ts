...
export function ensureCarriedLocation(db: any, characterId: number): number {
  const existing = db.prepare(
    "SELECT id FROM storage_locations WHERE character_id = ? AND type = 'carried'"
  ).get(characterId);
...
export async function locationRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (
      ...
      const maxOrder = (db.prepare(
        'SELECT MAX(sort_order) as m FROM storage_locations WHERE character_id = ?'
      ).get(char.id) as any)?.m ?? 0;
      ...
      const row = db.prepare('SELECT * FROM storage_locations WHERE id = ?').get(info.lastInsertRowid) as any;

      bus.emitChange({ type: 'inventory:change', partyId: char.party_id, characterId: char.id, action: 'adjust' });
  ...
  app.delete(
    ...
    async (req: FastifyRequest<{ Params: { locId: string } }>, reply: FastifyReply) => {
      ...
      if (userId === null) return;
      const db = getDb();
      const loc = db.prepare('SELECT * FROM storage_locations WHERE id = ?').get(Number(req.params.locId)) as any;
      ...
      for (const item of itemsToMove) {
        ...
        if (existing) {
          ...
          // Just move it
          db.prepare('UPDATE inventory SET storage_location_id = ? WHERE id = ?')
            .run(carriedId, item.id);
        }
      }
